...
- **Startup smoke**: `scripts/smoke-start.mjs` (`npm run smoke:start`) spawns the *compiled* entry with a clean env, `PORT=0`, file mode in a temp data dir; asserts ready line, health, a full mock turn over SSE with contiguous `seq`, JSONL + `meta.json` on disk, `403 PATH_ESCAPES_ROOT` outside allowed roots, SIGTERM → exit 0, recovery on a second boot, and refusal of `HOST=0.0.0.0` without opt-in. `test/boot.test.ts` covers the same contract in-process from sources.
