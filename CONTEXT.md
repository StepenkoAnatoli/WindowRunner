...
- **Boot entry**: `packages/server/src/index.ts`, the executable behind `npm start`. Sequence: `loadServerConfig(process.env)` → `startServer(config)` → banner + ready line `windows-runner listening on http://HOST:PORT` → signal handlers. Exits 1 with `configuration error (VARIABLE): …` on any invalid setting; nothing falls back silently.
...
- **Startup smoke**: `scripts/smoke-start.mjs` (`npm run smoke:start`) spawns the *checkout bundle* `packages/server/dist/index.cjs` with a clean env, `PORT=0`, file mode in a temp data dir; asserts ready line, health, a full mock turn over SSE with contiguous `seq`, JSONL + `meta.json` on disk, `403 PATH_ESCAPES_ROOT` outside allowed roots, SIGTERM → exit 0, recovery on a second boot, and refusal of `HOST=0.0.0.0` without opt-in. `test/boot.test.ts` covers the same contract in-process from sources.
- **Distribution runtime**: `npm run build` emits the normal workspace tsc outputs and then `scripts/bundle-server.mjs` produces the self-contained `packages/server/dist/index.cjs` bundle (server + express + shared runtime). The root package publishes only that entry plus `postinstall`, `ensure-built`, docs and license files; `npm run smoke:runtime` installs the tarball outside the checkout, runs `npm start`, completes a mock SSE turn and checks shutdown. The unbundled workspace dist files are development/type seams, not consumer runtime dependencies.
