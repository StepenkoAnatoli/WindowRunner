...
echo "    npm run build        emit workspace dist/ + bundled server runtime"
echo "    npm run smoke:packed verify packed contents + bundle imports"
echo "    npm run smoke:runtime install tarball outside checkout and run npm start"
echo "    npm run smoke:start  boot the checkout bundle and run a turn"
echo ""
echo "  Not available yet (see docs/INSTALL.md):"
echo "    npx windows-runner   no bin, package unpublished (gaps G-01, G-05)"
echo "  Docker artifact is ready but not verified here (no Docker daemon/CI)."
echo ""
