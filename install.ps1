...
Write-Host "    npm run build         emit workspace dist/ + bundled server runtime"
Write-Host "    npm run smoke:packed  verify packed contents + bundle imports"
Write-Host "    npm run smoke:runtime install tarball outside checkout and run npm start"
Write-Host "    npm run smoke:start   boot the checkout bundle and run a turn"
Write-Host ""
Write-Host "  Not available yet (see docs/INSTALL.md):"
Write-Host "    npx windows-runner    no bin, package unpublished (gaps G-01, G-05)"
Write-Host "  Docker artifact is ready but not verified here (no Docker daemon/CI)."
Write-Host ""
