...
describe("FileSessionStore", () => {
  ...
  it("root rejection after configuration changes", async () => {
    ...
    // Boot with allowedRoots that does NOT include /tmp — should skip session
    ...
    // Boot with allowedRoots that includes /tmp — should load and clear activeTurnId
    const diagnostics2 = await store.boot(["/tmp"], () => Date.now());
  ...
  it("restart recovery clears activeTurnId and never trusts persisted roots for authorization", async () => {
    ...
    // Boot with current allowedRoots = ["/tmp"] — snapshot contains /etc but should not authorize /etc
    const diagnostics = await store.boot(["/tmp"], () => Date.now());
    ...
    // Now try boot with allowedRoots that doesn't include /tmp — should reject even though snapshot includes /tmp
    const meta2 = {
      version: 1 as const,
    ...
    // Should skip both sessions because /tmp not in current allowedRoots, even though snapshot says it is
